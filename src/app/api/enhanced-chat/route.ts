...
export async function POST(request: Request) {
  try {
    ...
    if (assessmentId) {
      ...
      // Prepare messages with context and assessment prompt
      const systemPrompt = assessmentPrompt ||
        'You are conducting an assessment. Ask thoughtful, open-ended questions to understand the user better.'

      const contextualMessages = [
        ...messages.slice(0, -1), // Previous conversation
        {
          role: 'system',
          content: contextContent ? `${systemPrompt}\n\nRelevant context:\n${contextContent}` : systemPrompt
        },
        {
          role: 'user',
          content: userMessage
